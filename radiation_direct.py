...
def solve_direct(mesh_name, domain_dimensions, k, delta, radius, tau, c, scale, BC, f, nonlinear):
    MMS = True
    ...
    u_.assign(0) 
    if nonlinear:
        eps_list = np.linspace(0.0, 1.0, 1000)
        tau = Constant(0.0)
    else:
        eps_list = [0.25]
    ...
    for eps in eps_list:
        ...
        xx = Constant(eps * 4)
        F = k * inner(grad(u), grad(v)) * dx - f * v * dx + g * v * flux_bdys
        for i in range(num):
